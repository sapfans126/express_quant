...
TDX_FILE_FORMAT: Dict[str, Columns] = {
    "day": Columns(
        ...
        scale_factors=[1.0, 0.001, 0.001, 0.001, 0.001, 1.0, 1.0, 1.0, 1.0]
    ),
    "min": Columns(
        name="min",
        sub_dir = "lday",
...
class TdxFormatter:
    @staticmethod
    def get_tdxfile_format(file_type: str) -> Optional[Tuple[np.dtype, List[float]]]:
        """
        获取通达信文件格式定义
        ...
        Returns:
            元组 (dtype, scale_factors) 包含NumPy数据类型和缩放系数列表
            如果类型不存在则返回 None
        """
        # 统一文件类型为小写，增强兼容性
...
if __name__ == "__main__":
    t,d,s = TdxFormatter.get_tdxfile_format('day')
    print(t)
    print(d)
    print(s)
