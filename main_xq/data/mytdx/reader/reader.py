...
class Reader:
    ...
    def read_hist_quote(self,
        ...
        # 检查结果
        if clean_code.get("error"):  # 假设返回字典包含 error 字段
            print(f"❌ 证券代码解析失败：{clean_code['error']}")
            # 处理错误
            return None
        else:
            print(f"✅ 证券代码解析成功：{clean_code['code']}")
            # 正常处理

        # 獲取文件類型格式，及欄位縮放比例
        try:
            fmt_dtype, fmt_scale_factors \
                = fmt.TdxFormatter.get_tdxfile_format(file_type=period)
